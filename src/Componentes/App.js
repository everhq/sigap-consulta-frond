...
import tramites from './tramite-row';
import FormRegistroTramitesScreen from './formularioRegistroTramites.js';
...
class App extends React.Component {
 ...
 //   this.arreglosReporte = this.arreglosReporte.bind(this);
    ...
    /*const App1 = createStackNavigator({
      regTramites: { screen: FormRegistroTramitesScreen },
    });*/
    ...
    fetch(CONFIG+'concepto/leer/restringido/' + nombrenuevo)
      ...
      fetch(CONFIG+'/beneficio/listar/' + nombrenuevo)
        .then((response)=>{
          ...
          console.log("datos");
          console.log(datos);
          console.log("Llegamos al tramite");
...
*/
  ...
  registrarTramite=(e)=>{
        ...
        browserHistory.push('/vista/tramite-m/'+this.state.name);
        e.preventDefault();
    ...
    if (this.state.pagos.length > 0) {
      return (
        ...
        <div>
          ...
          <ul id="nav-mobile" className=" row left  hide-on-med-and-down">              
              <li ><a className="seleccionar col" onClick={this.registrarTramite} >Registrar Trámite<i className="material-icons right">edit</i></a></li>
...
enviar2=(e)=>{
  ...
  if(this.state.aparecer/*Tramite*/){
    this.setState({
      aparecer/*Tramite*/:false,
    });

  }
  else{
    this.setState({
      aparecer/*Tramite*/:true,
    });
    window.location.reload();
  }
}
